...
abstract class Validators {
  ...
  static FormFieldValidator<String> phone(String messageKey) {
    return (value) {
      ...
      var reg = RegExp(r'^\([0-9]{2}\) [0-9]{5}\-[0-9]{4}$');
  ...
  static FormFieldValidator<String> min(int min, String messageKey) {
    return (value) {
      if (value?.isEmpty ?? true) return null;
      if ((value?.length ?? 0) < min) return messageKey.translate();
      return null;
    };
  }
  ...
  static FormFieldValidator<String> max(int max, String messageKey) {
    return (value) {
      if (value?.isEmpty ?? true) return null;
      if ((value?.length ?? 0) > max) return messageKey.translate();
      return null;
    };
  }

  /// Validar se o campo está entre a quantidade [minima] e [maxima]
  /// de caracteres
  ...
  static FormFieldValidator<String> date(String messageKey) {
    return (value) {
      ...
      var day = int.tryParse(components[0]) ?? 0;
      var month = int.tryParse(components[1]) ?? 0;
      var year = int.tryParse(components[2]) ?? 0;
